...
impl<N: BaseFloat> Ahrs<N> {

  /// Updates the current state quaternion using 9dof IMU values.
  pub fn update( &mut self, gyroscope: Vector3<N>, accelerometer: Vector3<N>, magnetometer: Vector3<N> ) -> bool {
    let q = self.quat;
  ...
  pub fn update_imu( &mut self, gyroscope: Vector3<N>, accelerometer: Vector3<N> ) -> bool {
    let q = self.quat;
