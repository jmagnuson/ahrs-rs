...
impl AHRS {
  ...
  pub fn default() -> AHRS {
    AHRS { sample_period: (1.0f64)/(256.0f64), quat: Quat::new(1.0f64, 0.0, 0.0, 0.0),
           kp: 1.0f64, ki: 0.0f64, kp_init: 1.0, init_period: 5.0f64, q: Quat::new(1.0f64, 0.0, 0.0, 0.0),
           int_error: Vec3::new(0.0f64, 0.0, 0.0), kp_ramped: 0.0f64, beta: 1.0f64
         }
  }
  ...
  pub fn update( &mut self, gyroscope: Vec3<f64>, accelerometer: Vec3<f64>, magnetometer: Vec3<f64> ) -> bool {
    ...
    let h = self.q * ( Quat::new(0.0f64, magnetometer[0], magnetometer[1], magnetometer[2]) * self.quat );
  ...
  pub fn update_imu( &mut self, gyroscope: Vec3<f64>, accelerometer: Vec3<f64> ) -> bool {
    ...
    //  self.int_error = Vec3::new(0.0f64, 0.0, 0.0);
    ...
    let Ref:Vec3<f64> = gyroscope - error*self.kp - self.int_error*self.ki;
